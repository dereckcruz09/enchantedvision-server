...
CREATE TABLE IF NOT EXISTS revocation_epoch (
    id          INTEGER PRIMARY KEY CHECK (id = 1),
    epoch_time  INTEGER NOT NULL DEFAULT 0
);

INSERT OR IGNORE INTO revocation_epoch (id, epoch_time) VALUES (1, 0);
"""
...
class KeyStore:
    ...
    def get_revocation_epoch(self) -> int:
        """Get the current revocation epoch timestamp."""
        with self._conn() as c:
            row = c.execute("SELECT epoch_time FROM revocation_epoch WHERE id = 1").fetchone()
        return row["epoch_time"] if row else 0

    def set_revocation_epoch(self, epoch_time: int) -> None:
        """Set the revocation epoch to invalidate all tokens issued before this time."""
        with self._conn() as c:
            c.execute("UPDATE revocation_epoch SET epoch_time = ? WHERE id = 1", (epoch_time,))
