...
def refresh(payload: RefreshIn):
    ...
    # Check if token was issued before the revocation epoch (instant revocation)
    revocation_epoch = STORE.get_revocation_epoch()
    if iat < revocation_epoch:
        raise HTTPException(status_code=403, detail="key revoked")
...
@app.post("/admin/revoke-all-instant", dependencies=[Depends(require_admin)])
def admin_revoke_all_instant():
    """
    Instantly revoke all tokens by setting the revocation epoch to now.
    This invalidates all tokens issued before this moment.
    """
    import time
    now = int(time.time())
    STORE.set_revocation_epoch(now)
    return {"revoked_all": True, "epoch": now}
